...
| T91a — TypeScript marker types + zod schemas | `[ ]` | `tasks/T91a-marker-types-and-schemas.md`. New `src/types/markers.ts` mirroring the four Rust enums, plus zod schemas. Wire format is internally tagged: `#[serde(tag = "kind", rename_all = "snake_case")]`, so variants are `point`/`spot`/`directional`, `play`/`play_once`, `ambient`/`one_shot`, `particle`/`billboard` — snake_case, and field names cross IPC verbatim. Blocks T91b–T91d. |
| T91b — Store: marker fields + update actions | `[ ]` | `tasks/T91b-marker-store-actions.md`. `SceneObject` gains four optional marker fields; adds `updateObjectLight` / `updateObjectAnimation` / `updateObjectAudio` / `updateObjectVfx`. Removal must `delete` the key rather than set `undefined`, to honour the Rust `skip_serializing_if = "Option::is_none"` contract. Depends on T91a. |
| T91c — Inspector marker panels | `[ ]` | `tasks/T91c-inspector-marker-panels.md`. Four new panel files under `src/components/Inspector/`. `Inspector.tsx` is already 591 lines, so the anti-godfile rule applies: extract, do not extend. Switching a marker's `kind` must always yield a schema-valid variant. Depends on T91a + T91b; parallel with T91d. |
| T91d — Thread markers through the export payload | `[ ]` | `tasks/T91d-marker-export-wiring.md`. The Rust exporter already reads the marker fields, but the frontend never sends them, so the feature is dead end-to-end. Threads markers through the payload builders in `ExportPanel.tsx` and `FileMenu.tsx`. Objects without a marker must omit the key entirely — never `null`. Depends on T91a + T91b; parallel with T91c. |
