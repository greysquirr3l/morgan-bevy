  ...
  const handleMouseUp = () => {
    ...
    selectedObjects.forEach(objectId => {
      ...
      if (obj && initial) {
        ...
        const positionChanged = !obj.position.every(
          (val, i) => Math.abs(val - initial.position[i]) < 0.001
        )
        const rotationChanged = !obj.rotation.every(
          (val, i) => Math.abs(val - initial.rotation[i]) < 0.001
        )
